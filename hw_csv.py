...
def print_rep(res: dict):
    print(
        "\n\tНазвание отдела \t|  Численность\t|\tВилка зарплат\t|\tСредняя зп"
    )
    for key, val in res.items():
        avg_sal = float(
            "{:.2f}".format(val[3] / val[0])
        )  # т.к зп может быть только в руб. и коп., то откидываем лишнее
...
if __name__ == "__main__":
    ...
    При этом убедитесь, что он называется 'work.csv'.\n"""
    )
